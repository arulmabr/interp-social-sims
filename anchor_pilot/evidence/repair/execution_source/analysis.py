...
def condition_summary(rows, records, baseline, bootstrap=0, planted=None):
    ...
    result = {'n': len(rows), 'scientific_classification': None,
              ...
              'answer_mass_below_0_95_count': sum(r['answer_mass'] < .95 for r in records),
              'mean_answer_mass': float(np.mean([r['answer_mass'] for r in records]))}
    patched = [r for r in records if 'actual_norm' in r]
    if patched:
        result['displacement'] = {
            'rows': len(patched),
            'mean_actual_norm': float(np.mean([r['actual_norm'] for r in patched])),
            'max_relative_norm_error': max(abs(r['actual_norm']/r['requested_norm']-1) for r in patched),
            'positions_per_prompt_recorded': sorted({r['positions_per_prompt'] for r in patched
                                                      if 'positions_per_prompt' in r}) or None,
            'position_scope_source': 'Execution manifest and final-token hook; legacy records omit the position-count field.',
        }
    ...
    result['dominance'] = {'rows': len(dominant),
        ...
        'note': 'A stochastic CPT agent can choose a dominated option with nonzero probability. Argmax here is conditional on A/B, not a full-vocabulary generated answer; inspect answer mass alongside this statistic.'}
    if planted is not None and test:
        target = probabilities([rows[i] for i in test], planted)
        masses = np.array([records[i]['answer_mass'] for i in test]).clip(1e-12, 1)
        full_ce = cross_entropy(target, p[test]) - float(np.mean(np.log(masses)))
        ...
        result['planted_recovery'] = {'truth': planted,
            ...
            'full_vocabulary_cross_entropy': full_ce, 'target_entropy': entropy,
            'full_vocabulary_excess_cross_entropy': full_ce - entropy}
    return result
