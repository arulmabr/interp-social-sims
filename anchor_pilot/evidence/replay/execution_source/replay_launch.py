...
'''
    ...
    files += [(root/'outputs/replay-plan-v1'/name,'anchor_pilot/outputs/replay-plan-v1/'+name) for name in ('manifest.json','rows.json')]
...
with (root/('session-'+p['session']+'.log')).open('x') as log:
    job=subprocess.Popen([sys.executable,'-u','-c',command,str(root/('exit-'+p['session']+'.txt')),str(root/'run_replay.sh')],env=env,stdin=subprocess.DEVNULL,stdout=log,stderr=log,start_new_session=True)
