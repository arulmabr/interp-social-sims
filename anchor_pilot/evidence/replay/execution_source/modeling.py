...
def load_stack(fixture=False, token_name=None):
    ...
    # Approximately equal base-weight allocation. The repair adapts blocks on
    # both devices; the backward path crosses GPU boundaries explicitly.
...
def prepare_inputs(model, tokenizer, rows, response_format=None):
    ...
    for row in rows:
        if tokenizer is None:
            ...
            spec = response_format or {}
            messages = []
            if spec.get('system'):
                messages.append({'role': 'system', 'content': spec['system']})
            messages.append({'role': 'user', 'content': row['text']})
            if spec.get('prefill'):
                messages.append({'role': 'assistant', 'content': spec['prefill']})
                rendered = tokenizer.apply_chat_template(messages, tokenize=False, continue_final_message=True)
            else:
            ...
            ids = {}
            for label in 'AB':
                continuation = tokenizer.encode(rendered + spec.get('continuation_prefix', '') + label,
...
def add_lora(model, first_layer, last_layer, rank=8, target_modules=None):
...
def save_adapter(model, config, destination):
    ...
    temporary = destination / 'adapter.pt.tmp'
    torch.save({k: v.detach().cpu() for k, v in get_peft_model_state_dict(model).items()}, temporary)
    temporary.replace(destination / 'adapter.pt')
