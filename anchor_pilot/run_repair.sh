...
python -m pytest -q anchor_pilot/test_core.py anchor_pilot/test_cpt.py anchor_pilot/test_modeling.py anchor_pilot/test_repair.py
...
if (( remaining <= 1800 )); then
  echo 'Insufficient time before the backup cutoff; leaving the GPU experiment unopened.'
  exit 2
fi
run_dir="anchor_pilot/outputs/repair-$(date -u +%Y%m%dT%H%M%SZ)"
timeout --signal=TERM --kill-after=30s "${remaining}s" python -u -m anchor_pilot.repair \
  --output "$run_dir" --max-seconds "$remaining" --reserve-seconds 1800
