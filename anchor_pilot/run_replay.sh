...
python -m pytest -q anchor_pilot/test_core.py anchor_pilot/test_cpt.py anchor_pilot/test_modeling.py anchor_pilot/test_repair.py anchor_pilot/test_replay.py
...
run_dir="anchor_pilot/outputs/replay-$(date -u +%Y%m%dT%H%M%SZ)"
timeout --signal=TERM --kill-after=30s "${remaining}s" python -u -m anchor_pilot.replay --output "$run_dir"
